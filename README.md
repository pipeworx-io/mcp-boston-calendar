...
`tools/list` at `https://gateway.pipeworx.io/boston-calendar/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/boston_calendar_events \
  -H 'Content-Type: application/json' \
  -d '{"query":"music"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/boston_calendar_events`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "boston-calendar": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-boston-calendar"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-boston-calendar
```
