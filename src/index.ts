...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'The Boston Calendar');
}
...
async function fetchEvents(): Promise<RawEvent[]> {
  const res = await pwFetch(FEED, { headers: { Accept: 'application/json', 'User-Agent': UA } });
  if (!res.ok) throw await httpError(res, 'Boston Calendar');
